...
if __name__ == "__main__":
    ...
    def reloadAndRun(n=10000,monitoring_freq=10):
        supervisor.reset()
        A = ata.fitsread(simname+"_A.fits").astype(np.float32)
        L = ata.fitsread(simname+"_L.fits").astype(np.float32)
        K = ata.fitsread(simname+"_K.fits").astype(np.float32)
        G = ata.fitsread(simname+"_G.fits").astype(np.float32)
        print("""
        Starting %d loops
        """ %(n,))
        sim.loopLQG(n,monitoring_freq=monitoring_freq,A=A,L=L,K=K,G=G)
